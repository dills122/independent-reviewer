...
function requireUniqueFindingIds(findings: Array<{ id: string }>, context: z.RefinementCtx): void {
...
const PreliminaryAssessmentBaseV1Schema = z.strictObject({
...
export const PreliminaryAssessmentV1Schema = PreliminaryAssessmentBaseV1Schema.superRefine(
  validatePreliminaryStructure,
);

const FinalReviewReportBaseV1Schema = z.strictObject({
  schemaVersion: z.literal(1),
...
export function validateReportStructure(
  report: Omit<
    z.infer<typeof FinalReviewReportBaseV1Schema>,
    "schemaVersion" | "findings" | "verdict"
  > & { findings: { id: string; severity: string }[]; verdict: string },
  context: z.RefinementCtx,
): void {
  ...
  const blockingFinding = report.findings.some(
    (finding) =>
      finding.severity === "P0" || finding.severity === "P1" || finding.severity === "REQUIRED",
  );
...
export const FinalReviewReportV1Schema =
  FinalReviewReportBaseV1Schema.superRefine(validateReportStructure);
