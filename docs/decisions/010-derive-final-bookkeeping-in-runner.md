...
3. Non-blocking finding corrections and reviewer suggestions become fast follows;
   a non-empty set produces `READY_WITH_FOLLOW_UPS`.
...
Provider candidate v3 retains `verdict` and `nextActions.blockers` for wire and resume
compatibility. Runner treats both as non-authoritative and discards them. A future
candidate version may remove those fields without changing persisted report v1 or CLI
exit semantics.
...
### Remove all model uncertainty judgments

Rejected. Findings, limitations, concern dispositions, and optional suggestions remain
review judgments. Runner classifies them but does not invent whether code is defective.
